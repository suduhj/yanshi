...
import { completeDailyTodayAction, togglePlannedTodayAction } from "@/lib/tasks/actions";
import {
  DUE_FILTER_LABELS,
  DUE_FILTERS,
  TASK_STATUS_LABELS,
  ...
  return (
    <main className="mx-auto flex min-h-screen w-full max-w-6xl flex-col gap-6 px-4 py-6 sm:px-6 lg:px-8">
      <header className="flex flex-col gap-3 border-b border-neutral-200 pb-5 xl:flex-row xl:items-end xl:justify-between">
        ...
        <div className="grid grid-cols-2 gap-2 text-center text-sm sm:grid-cols-3 xl:w-[760px] xl:grid-cols-6">
          ...
          <SummaryItem label="今日要做" value={summary.plannedToday} />
          <SummaryItem label="每日任务" value={summary.daily} />
      ...
      <section className="grid gap-6 lg:grid-cols-[360px_1fr]">
        ...
        <section className="flex min-w-0 flex-col gap-4">
          ...
          />
          <TaskSection
            emptyText="今日计划还是空的。可以在任务卡片上点击“加入今日”，把今天主动安排的事放进来。"
            tasks={sections.plannedToday}
            title="今日要做"
          />
          <TaskSection
            emptyText="暂无每日任务。像背单词、检查学习通、查看课程群这类每日事项可以勾选“每日任务”。"
  ...
  return (
    <li className="grid gap-4 px-4 py-4 md:grid-cols-[1fr_150px]">
      <div className="min-w-0">
        ...
        <p className="mt-1 text-sm text-neutral-500">{formatDueDistance(task.dueAt)}</p>
        {task.nextAction ? (
